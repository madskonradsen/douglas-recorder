...
// Helper function to get center of element for meta information
function getCenterOfElement(elem) {
    ...
    const x = left + (elem.clientWidth / 2)
    const y = top + (elem.clientHeight / 2)
    return {
        y: Math.round(y),
        x: Math.round(x) 
    };
...
// Register the events and provide handlers
...
function handle(action, node, value, meta) {
  if (chrome && chrome.runtime) {
    var generator = new CssSelectorGenerator;
    var path = generator.getSelector(node);
    var message = {
      "action": action, 
      "path": path,
      "value": value
...
// START CSS SELECTOR GENERATOR LIBRARY - https://github.com/fczbkk/css-selector-generator
(function() {
